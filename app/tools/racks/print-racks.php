        ...
        foreach ($Racks->all_racks as $r) {
            ...
            if (empty($rack_devices)) {
                ...
                foreach ($rack_devices as $k=>$d) {
                    ...
                    if($admin) {
                        print "<a href='' class='btn btn-xs btn-default btn-danger editRackDevice' data-action='remove' rel='tooltip' data-html='true' data-placement='left' title='"._("Remove")."' data-action='remove' style='margin-bottom:2px;margin-right:5px;' data-rackid='$r->id' data-deviceid='$d->id'><i class='fa fa-times'></i></a> ";
