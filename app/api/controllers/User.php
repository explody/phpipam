...
class User_controller extends Common_api_functions
{
    ...
    public function __construct($Database, $Tools=null, $params=null, $Response)
    {
        ...
        if (isset($_SERVER['HTTP_TOKEN'])&&!isset($_SERVER['HTTP_PHPIPAM_TOKEN'])) {
            $_SERVER['HTTP_PHPIPAM_TOKEN'] = $_SERVER['HTTP_TOKEN'];
        }
    }
    ...
    private function validate_block()
    {
        ...
        if ($this->block_ip === true) {
            ...
            // failure
            if ($cnt > $this->max_failures) {
                $this->Response->throw_exception(500, "Your IP has been blocked for 5 minutes because of excesive login failures");
            }
        }
    }
    ...
    public function set_token_valid_time($token_valid_time = null)
    {
        ...
        if ($this->token_length!=null) {
            if (!is_numeric($this->token_length)) {
                $this->Response->throw_exception(500, "token valid time must be an integer");
            }
        }
        // save
    ...
    public function set_max_failures($failures=null)
    {
        ...
        if ($this->token_length!=null) {
            if (!is_numeric($this->token_length)) {
                $this->Response->throw_exception(500, "Max failures must be an integer");
            }
        }
        // save
    ...
    public function set_token_length($length = null)
    {
        ...
        if ($length!=null) {
            if (!is_numeric($length)) {
                $this->Response->throw_exception(500, "token length must be an integer");
            } elseif ($length>24) {
                $this->Response->throw_exception(500, "Maximum token length is 24 characters");
            }
        }
        // save
    ...
    {
        // is set
        if (strlen($this->User->user->token)==0) {
            return false;
        }
        // date not set
        elseif (strlen($this->User->user->token_valid_until)==0) {
            return false;
        }
        // expired
        elseif ($this->User->user->token_valid_until < date("Y-m-d H:is:")) {
            return false;
        }
        // ok
        else {
    ...
    private function validate_requested_token_user()
    {
        ...
        if (!isset($_SERVER['HTTP_PHPIPAM_TOKEN'])) {
            $this->Response->throw_exception(403, "Please provide token");
        }
