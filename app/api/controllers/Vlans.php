...
class Vlans_controller extends Common_api_functions
{
    ...
    public function GET()
    {
        ...
        if (!isset($this->_params->id)) {
            ...
            // check result
            if ($result===false) {
                $this->Response->throw_exception(404, 'No vlans configured');
            } else {
        ...
        // read vlan details
            ...
            // check result
            if ($result==null) {
                $this->Response->throw_exception(404, "Vlan not found");
            } else {
                return array("code"=>200, "data"=>$this->prepare_result($result, null, true, true));
            }
        }
    ...
    private function validate_vlan()
    {
        // validate id
        if (!isset($this->_params->id)) {
            $this->Response->throw_exception(409, "Vlan Id is required");
        }
        // validate number
        if (!is_numeric($this->_params->id)) {
            $this->Response->throw_exception(409, "Vlan Id must be numeric");
        }
        // check that it exists
        if ($this->Tools->fetch_object("vlans", "vlanId", $this->_params->id) === false) {
            $this->Response->throw_exception(404, "Invalid Vlan id");
        }
    }
    ...
    private function validate_vlan_edit()
    {
        ...
        if ($_SERVER['REQUEST_METHOD']=="POST") {
            if ($this->_params->number<0) {
                $this->Response->throw_exception(409, "Vlan number cannot be negative");
            } elseif (!is_numeric($this->_params->number)) {
                $this->Response->throw_exception(409, "Vlan number must be number");
            }
        }
        if (strlen($this->_params->name)==0) {
            $this->Response->throw_exception(409, "Vlan name is required");
        }
    }
    ...
    {
        // validate id
        if (!isset($this->_params->domainId)) {
            $this->_params->domainId = 1;
        }
        // validate number
        if (!is_numeric($this->_params->domainId)) {
            $this->Response->throw_exception(409, "Domain id must be numeric");
        }
        // check that it exists
