...
class Addresses_controller extends Common_api_functions
{
    ...
    private function validate_tag()
    {
        // numeric
        if (!is_numeric(@$this->_params->id2)) {
            $this->Response->throw_exception(409, 'Invalid tag identifier');
        }
        // check db
    ...
    private function validate_subnet()
    {
        ...
        if (!is_numeric($this->_params->subnetId)) {
            $this->Response->throw_exception(409, "Subnet Id must be numeric");
        }
        // check subnet
        if (is_null($res = $this->Subnets->fetch_subnet("id", $this->_params->subnetId))) {
            $this->Response->throw_exception(404, "Invalid subnet Id");
        } else {
    ...
    public function validate_create_parameters()
    {
        ...
        if (strlen($this->_params->mac)>0) {
            if ($this->validate_mac($this->_params->mac)===false) {
                $this->Response->throw_exception(409, "Invalid MAC address");
            }
            // normalize
            else {
        ...
        if (isset($this->_params->switch)) {
            if ($this->Tools->fetch_object("devices", "id", $this->_params->switch)===false) {
                $this->Response->throw_exception(400, "Device does not exist " . $this->_params->switch);
            }
        }
        ...
        if (isset($this->_params->state)) {
            if ($this->Tools->fetch_object("ipTags", "id", $this->_params->state)===false) {
                $this->Response->throw_exception(404, "Tag does not exist");
            }
        } else {
