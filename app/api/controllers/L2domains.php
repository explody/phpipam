...
class L2domains_controller extends Common_api_functions
{
    ...
    public function OPTIONS()
    {
        ...
        $result['methods'] = array(
                                array("href"=>"/api/".$this->_params->app_id."/l2domains/",        "methods"=>array(array("rel"=>"options", "method"=>"OPTIONS"))),
                                array("href"=>"/api/".$this->_params->app_id."/l2domains/{id}/",    "methods"=>array(array("rel"=>"read",    "method"=>"GET"),
    ...
    private function validate_domain_edit()
    {
        ...
        if ($_SERVER['REQUEST_METHOD']=="DELETE") {
            ...
            if (@$this->_params->id==1 && $_SERVER['REQUEST_METHOD']=="DELETE") {
                $this->Response->throw_exception(409, "Default domain cannot be deleted");
            }
            // ID must be numeric
            if (!is_numeric($this->_params->id)) {
                $this->Response->throw_exception(404, "Invalid domain id");
        ...
        elseif ($_SERVER['REQUEST_METHOD']=="POST") {
            ...
            if (@$this->_params->name == "" || !isset($this->_params->name)) {
                $this->Response->throw_exception(409, "Domain name is mandatory");
            }
        }
        // update checks
        elseif ($_SERVER['REQUEST_METHOD']=="PATCH") {
            // ID must be numeric
            if (!is_numeric($this->_params->id)) {
                $this->Response->throw_exception(409, "Invalid domain id");
            }
            // name must be present
            if (@$this->_params->name == "" && isset($this->_params->name)) {
                $this->Response->throw_exception(409, "Domain name is mandatory");
