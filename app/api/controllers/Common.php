...
class Common_api_functions
{
    ...
    protected function prepare_result($result, $controller = null, $links = true, $transform_address = true)
    {
        ...
        if ($transform_address) {
            $result = $this->transform_address($result);
        }
    ...
    protected function filter_result($result)
    {
        ...
        # null?
        if (sizeof($result)==0) {
            $this->Response->throw_exception(404, 'No results (filter applied)');
        }
    ...
    protected function transform_address($result)
    {
        ...
        if (is_array($result)) {
            foreach ($result as $k=>$r) {
                ...
                // transform
                if (isset($r->subnet)) {
                    $r->subnet  = $this->Subnets->transform_address($r->subnet, "dotted");
                } elseif (isset($r->ip_addr)) {
                    $r->ip_addr = $this->Subnets->transform_address($r->ip_addr, "dotted");
                }
            }
        }
        // single item
            ...
            // remove IP
                ...
                if (isset($result->subnet)) {
                    $result->subnet  = $this->Subnets->transform_address($result->subnet, "dotted");
                } elseif (isset($result->ip_addr)) {
                    $result->ip_addr = $this->Subnets->transform_address($result->ip_addr, "dotted");
                }
        }
