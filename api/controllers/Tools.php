...
class Tools_controller extends Common_api_functions
{
    ...
    {
        // deviceTypes
        if ($this->_params->id == "vlans") {
            $this->sort_key = "vlanId";
        } elseif ($this->_params->id == "vrf") {
            $this->sort_key = "vrfId";
        } else {
            $this->sort_key = "id";
        }
    ...
    public function GET()
    {
        ...
        if (!isset($this->_params->id2)) {
            # TODO: fetch_all_bjects should return empty sets, not false
            $result = $this->Tools->fetch_all_objects($this->_params->id, $this->sort_key);
            // result
            if ($result===false) {
                $this->Response->throw_exception(404, 'No objects found');
            } else {
                return array("code"=>200, "data"=>$this->prepare_result($result, "tools/".$this->_params->id, true, false));
            }
        }
        # by parameter
        elseif (isset($this->_params->id3)) {
            ...
            return array("code"=>200, "data"=>$this->prepare_result($result, "tools/".$this->_params->id, true, true));
        }
        # by id
        else {
            // numeric
            if (!is_numeric($this->_params->id2)) {
                $this->Response->throw_exception(404, 'Identifier must be numeric');
            }
    ...
    private function validate_subcontroller()
    {
        ...
        if ($_SERVER['REQUEST_METHOD']!=="OPTIONS") {
            if (!in_array($this->_params->id, @$this->subcontrollers)) {
                $this->Response->throw_exception(400, "Invalid subcontroller");
            }
        }
    }
    ...
    private function validate_subcontroller_identifier()
    {
        ...
        if (isset($this->_params->id3)) {
            if (!in_array("id3", $this->identifiers[$this->_params->id])) {
                $this->Response->throw_exception(400, "Invalid subcontroller identifier");
            }
        }
        // id2
        if (isset($this->_params->id2)) {
            if (!in_array("id2", $this->identifiers[$this->_params->id])) {
                $this->Response->throw_exception(400, "Invalid subcontroller identifier");
            }
        }
    }
    ...
    private function validate_device_type()
    {
        if ($this->_params->id == "devices" && isset($this->_params->type)) {
            ...
            if (!is_numeric($this->_params->type)) {
                $this->Response->throw_exception(409, "Invalid devicetype identifier");
            }
            // check
