...
class Vrfs_controller extends Common_api_functions
{
    ...
    public function GET()
    {
        ...
        if (!isset($this->_params->id)) {
            ...
            // check result
            if ($result===false) {
                $this->Response->throw_exception(404, 'No vrfs configured');
            } else {
        ...
        // by id
            ...
            // check result
            if ($result==null) {
                $this->Response->throw_exception(404, "VRF not found");
            } else {
                return array("code"=>200, "data"=>$this->prepare_result($result, null, true, true));
            }
        }
    ...
    private function validate_vrf()
    {
        // validate id
        if (!isset($this->_params->id)) {
            $this->Response->throw_exception(409, "Vrf Id is required");
        }
        // validate number
        if (!is_numeric($this->_params->id)) {
            $this->Response->throw_exception(409, "Vrf Id must be numeric");
        }
        // check that it exists
        if ($this->Tools->fetch_object("vrf", "vrfId", $this->_params->id) === false) {
            $this->Response->throw_exception(404, "Invalid VRF id");
        }
    }
    ...
    private function validate_vrf_edit()
    {
        ...
        if ($_SERVER['REQUEST_METHOD']=="POST") {
            ...
            if (strlen($this->_params->name)==0) {
                $this->Response->throw_exception(409, "VRF name is required");
            }
            // check that it exists
            if ($this->Tools->fetch_object("vrf", "name", $this->_params->name) !== false) {
                $this->Response->throw_exception(409, "VRF with that name already exists");
            }
        }
        // update check
        else {
            ...
            if (isset($this->_params->name)) {
                if ($this->_params->name != $vrf_old->name) {
                    if ($this->Tools->fetch_object("vrf", "name", $this->_params->name)) {
                        $this->Response->throw_exception(409, "VRF with that name already exists");
                    }
                }
            }
